...
% Uses MAXFLOWDATA.M and RITA.M.
maxflowdata
nrnod=length(xpos);
mflow=0;
done=0;
scrsz = get(groot,'ScreenSize');
figure('Position',[1 scrsz(4)/2 scrsz(3)/2 scrsz(4)/2],'Name','Original network','NumberTitle','off')
rita2;
while ~done,
  fprintf(' When finished, input new path = []');
  path=tryinput(' Input new path (e.g. [10 5 6 7 8 9]) = ');
  if (length(path)~=0),
    minflow=Inf;
    ...
    if (path(1)~=start|path(length(path))~=stop | min(path)<1|max(path)>nrnod)
      ...
      if minflow<=0, 
        ...
        figure('Position',[scrsz(3)/2 scrsz(4)/2 scrsz(3)/2 scrsz(4)/2],'Name','Updated network','NumberTitle','off')
        rita2;
      end;
    end;
  else  
